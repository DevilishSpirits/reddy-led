...
int animation::parser::yajl_double(void *ctx, double doubleVal)
{
	...
	switch (parser->current_key) {
		...
		case KEY_INIT_STEPS:
			if (doubleVal > 1) {
				parser->parse_error = "\"init_steps\" values are in the [0..1] range (initial position ratio in the initial stop).";
				return 0;
			} else return parser->emit_initial_remaining(1-doubleVal);
		case KEY_NEXT_INDEX:
		case KEY_INIT_INDEXES: {
			double i;
...
int animation::parser::yajl_map_key(void *ctx, const unsigned char *key_u, size_t stringLen)
{
	...
	if (parser->tree_state == TREE_ROOT_MAP) {
		...
		else if (!strncmp(key,"init_steps",stringLen))
			parser->current_key = KEY_INIT_STEPS;
...
int animation::parser::yajl_start_map(void *ctx)
{
	...
	switch (parser->current_key) {
		case KEY_INIT_INDEXES:
			...
			return 1;
		case KEY_INIT_STEPS:
			parser->tree_state = TREE_INIT_STEPS;
			return 1;
		case KEY_NONE:
